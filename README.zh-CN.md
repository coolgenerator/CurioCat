...
7. **统计交叉验证** — 如果用户上传了 CSV/Excel 指标数据，CurioCat 会对 LLM 推断的因果边运行 Granger 因果检验。每条边被标注为 `confirmed`（统计检验一致，p<0.05）、`unsupported`（未发现显著关系）或 `contradicted`（反向因果更强）。确认的边提升证据分；不支持/矛盾的边被降权
...
CurioCat 还提供独立的因果分析模式（`/causal`），用于快速、数据驱动的分析：
